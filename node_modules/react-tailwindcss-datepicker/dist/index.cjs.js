...
function getNumberOfDay(dayString, i18n, startWeekOn) {
    let number = 0;
    let startDateModifier = 7 - dayjs__default["default"]().locale(i18n).weekday(0).get("day");
    if (startWeekOn) {
        switch (startWeekOn) {
            case "mon":
                startDateModifier = 6;
...
const Week = () => {
    ...
    loadLanguageModule(i18n);
    let startDateModifier = dayjs__default["default"]().locale(i18n).weekday(0).get("day");
    if (startWeekOn) {
        switch (startWeekOn) {
            case "mon":
                startDateModifier = 1;
...
const Calendar = ({ date, onClickPrevious, onClickNext, changeMonth, changeYear }) => {
    // Contexts
    const { period, changePeriod, changeDayHover, showFooter, changeDatepickerValue, hideDatepicker, asSingle, i18n, startWeekOn } = React.useContext(DatepickerContext);
    loadLanguageModule(i18n);
    // States
...
const Input = () => {
    // Context
    const { primaryColor, period, dayHover, changeDayHover, calendarContainer, arrowContainer, inputText, changeInputText, hideDatepicker, changeDatepickerValue, asSingle, placeholder, separator, disabled, inputClassName, toggleClassName, readOnly, displayFormat, inputId, inputName } = React.useContext(DatepickerContext);
    // UseRefs
    ...
    return (React__default["default"].createElement(React__default["default"].Fragment, null,
        ...
        React__default["default"].createElement("button", { type: "button", ref: buttonRef, disabled: disabled, className: `absolute right-0 h-full px-3 text-gray-400 focus:outline-none disabled:opacity-40 disabled:cursor-not-allowed ${toggleClassName}` }, inputText ? React__default["default"].createElement(CloseIcon, { className: "h-5 w-5" }) : React__default["default"].createElement(DateIcon, { className: "h-5 w-5" }))));
};
