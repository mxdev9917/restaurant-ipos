...
interface EditSlideProps {
    handleModel: (value: boolean, slider_ID: string) => void; // Update this line
    slider_ID: string;
    // slider_url: string;
    // slider_visibility: string;
}
const editSlide: React.FC<EditSlideProps> = ({ handleModel,slider_ID }) => {
    const {  token } = useAuth();
    ...
    return (
        <div className="w-80 sm:w-96 h-fit flex flex-col bg-white rounded-md shadow-lg pb-2">
            ...
            <div className="w-full flex justify-between p-2.5 border-b-2">
                <p className="text-xl text-orange-500 font-semibold">ແກ້ໄຂພາບສະໄລ</p>
                <button onClick={() => handleModel(false,slider_ID)} className="p-1.5 hover:bg-gray-200 rounded-full">
            ...
            <div className="w-full flex justify-end px-3">
                <button onClick={handleEdit} className= "flex items-center bg-orange-500 hover:bg-orange-400 py-1 px-3 text-white rounded-md">
                    <HiPencilAlt  className="text-xl" />
                    {loading ?
                        <LoadingSpinner text="ແກ້ໄຂ" />
                        :
                        "ແກ້ໄຂ"
                    }
                </button>
