...
function Authen() {
    ...
    const formSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
        ...
        try {
            ...
            if (res.data.user_role === "user") {
                alertSuccess(navigate, "/sale", t("logInSuccess"), "success");
            } else if (res.data.user_role === "chef") {
                alertSuccess(navigate, "/manage/kitchen", t("logInSuccess"), "success");
            } else {
                alertSuccess(navigate, "/dashboard", t("logInSuccess"), "success");
            }
    ...
    return (
        <div className="w-screen h-screen flex text-white justify-center relative bg-cover" style={{ backgroundImage: `url(${blobSceneHaikei})` }}>
            <p className=" font-normal text-sm absolute text-white right-3 bottom-3 pb-2 md:pb-0">{t("Copyright")} @ IPOS | Bran IT</p>
            <div className=" h-screen flex flex-col  items-center md:justify-center ">
                ...
                <form
                    ...
                    <p className="text-4xl text-orange-500 font-semibold mt-5">{t("signIn")}</p>
                    <div className=" h-fit flex flex-col justify-end mt-7">
                        <label className="text-orange-500" htmlFor="">{t("user")}</label>
                        <input type="text" value={email} onChange={(e) => setEmail(e.target.value)} className="bg-gray-50 border border-gray-300 text-gray-900  rounded-lg focus:ring-orange-500 focus:border-orange-500 block w-full p-2.5 mb-2   " placeholder="...." required />
                    </div>
                    ...
                    <div className="flex justify-end">
                        ...
                        <Link className="px-2 py-1 hover:text-orange-500 text-gray-400" to={'#'}>{t("forgotPass")}</Link>
                    </div>
                    <button className="bg-orange-500 text-white py-2.5 rounded-md my-2 font-semibold">
                    {loading ? <LoadingSpinner text={t("signIn")} /> : t("signIn")}
                    </button>
                    <div className=" flex justify-end items-end h-full w-full ">
                        <p className="text-orange-500 text-xs">{t("version")}</p>
                    </div>
                </form>
                <div className="flex justify-end text-white  w-[350px] md:w-[450px] pt-5">
                    <div className="font-normal text-white flex text-sm">
                        ...
                        <p>|</p>
                        <Link className="focus:text-orange-500" to={'#'}>{t("PrivacyPolicy")}</Link>
                    </div>
                </div>
            </div>
